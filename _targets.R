...
list(
  ...
  tar_target(
    name = cmip_cropped,
    command = import_cmip_sst(cmip_file = "tester") ),
  tar_target(
    name = cmip_clim,
  ...
  tar_target(
    name = oisst_clim_91,
    command = import_oisst_clim(climatology_period = "1991-2020", os.use = "unix") ),
  tar_target(
    name = oisst_month_avgs,
    command = months_from_clim(clim_source = oisst_clim_91, month_layer_key = NULL) ),
  
  ####  SODA Bottom temp/salinity  ####
  tar_target(
    name = soda_cropped,
