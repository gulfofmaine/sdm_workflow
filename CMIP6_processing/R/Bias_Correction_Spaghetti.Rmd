...
clim_futures <- function(var_option){
  ...
  # combine
  cmip_all <- bind_rows(list(cmip_mean, cmip_5, cmip_95)) %>% 
    mutate(data_source = factor(data_source, levels = c("CMIP 5th Perc.", "CMIP Mean", "CMIP 95th Perc.")),
  ...
  # Build plot
  regional_comparison_plot <- ggplot() +
    geom_line(data = cmip_all, aes_string("date", var_option, color = "area")) +
    scale_color_gmri() +
    facet_grid(area~data_source) +
    labs(x = "", y = var_label, color = "") +
    theme(legend.position = "bottom")
