function (b_i, a_i, t_i, c_iz = rep(0, length(b_i)), e_i = c_iz[, 1], v_i = rep(0, length(b_i)), FieldConfig = c(Omega1 = "IID", Epsilon1 = "IID", Omega2 = "IID", Epsilon2 = "IID"), ObsModel_ez = c(PosDist = 1, Link = 0), OverdispersionConfig = c(eta1 = 0, eta2 = 0), RhoConfig = c(Beta1 = 0, Beta2 = 0, Epsilon1 = 0, Epsilon2 = 0), 
          VamConfig = c(Method = 0, Rank = 0, Timing = 0), Aniso = TRUE, 
          PredTF_i = rep(0, length(b_i)), covariate_data = NULL, X1_formula = ~0, 
          X2_formula = ~0, X1config_cp = NULL, X2config_cp = NULL, 
          X_contrasts = NULL, catchability_data = NULL, Q1_formula = ~0, 
          Q2_formula = ~0, Q1config_k = NULL, Q2config_k = NULL, spatial_list, 
          Network_sz = NULL, F_ct = NULL, F_init = 1, CheckForErrors = TRUE, 
          yearbounds_zz = NULL, Options = c(), Expansion_cz = NULL, 
          Z_gm = NULL, Version = FishStatsUtils::get_latest_version(package = "VAST"), 
  ...
  if (Covariates_created == FALSE) {
    if (!is.null(covariate_data)) {
      if ("formula" %in% names(alternate_inputs)) {
        ...
        X1_gctp = X2_gctp = aperm(outer(X1_gtp, rep(1, 
                                                    n_c)), c(1, 4, 2, 3))
      }
    }
  }
  if (Covariates_created == FALSE) {
    if (!is.null(covariate_data)) {
      ...
      X1_gctp = aperm(outer(X1_gtp, rep(1, n_c)), c(1, 
                                                    4, 2, 3))
      ...
      X2_gctp = aperm(outer(X2_gtp, rep(1, n_c)), c(1, 
                                                    4, 2, 3))
    }
  }
  if (Covariates_created == FALSE) {
